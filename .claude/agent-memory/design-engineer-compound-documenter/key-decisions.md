...
- **Digest page is a server component; interactive header cluster is a client island** – `(dashboard)/page.tsx` is a server component; the "Parse video" button, the "+" Popover menu, and modal wiring are isolated in `digest-header-actions.tsx` (client island). Keeps the page RSC-friendly with the smallest possible client boundary. Affects: page.tsx, digest-header-actions.tsx, any future Digest-screen additions (follow the same server/client split).
- **Popover-then-Dialog race fixed via controlled `open`/`onOpenChange` on both modals** – a `DialogTrigger` inside a Popover unmounts mid-click (Popover closes before Dialog opens); both YouTubeParseModal and TelegramForwardModal gained an optional controlled mode. The "+" menu drives them by React state (`open` flag), sidestepping the unmount race entirely. Affects: youtube-parse-modal.tsx, telegram-forward-modal.tsx, digest-header-actions.tsx, and the pattern for any future Popover-driven Dialog.
- **`YouTubeParseModal` parameterized with optional `title`/`description`/`placeholder` props** – YouTube defaults preserved; the "+" menu's "Parse URL" path passes article/GitHub wording, restoring the prototype's behavior where the same modal is re-titled depending on entry point. The IA specifies "Parse URL" as a distinct input channel from "Parse video", so the two paths must read differently to users. Affects: youtube-parse-modal.tsx, digest-header-actions.tsx, any future entry path that reuses the same parse modal.
- **Code-review fixes: dead `text-warning` class → `text-chart-3`; `PageHeader` made to wrap at phone width** – `text-warning` does not exist in the token set; corrected to `text-chart-3`. `PageHeader` gained `flex-wrap` so header actions do not clip at 390px. Affects: digest-header-actions.tsx, page-header.tsx, any future header that uses the actions slot.
